    ...
    for (int i = 0; i < TEX_COUNT; i++) {
        ...
        if (result == 0) {  // EasyX中，0表示成功
            ...
            if (width > 0 && height > 0) {
                ...
                printf("加载后尺寸为0! 使用替代贴图\n");
                CreatePlaceholderTexture(&tm->images[i], i);
            }
        }
        else {
            printf("loadimage返回%d! 使用替代贴图\n", result);
            CreatePlaceholderTexture(&tm->images[i], i);
        }
    }
